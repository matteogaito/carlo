...
@pytest.mark.asyncio
async def test_plan_subtasks_run_in_order_before_parent_completes() -> None:
    ...
    orchestrator = Orchestrator(engine, factory, runner)
    assert await orchestrator.run_next() == "CAR-2"
    async with factory() as session:
        assert (await session.get(Task, "CAR-1")).status == TaskStatus.IN_PROGRESS
    assert await orchestrator.run_next() == "CAR-3"
    async with factory() as session:
